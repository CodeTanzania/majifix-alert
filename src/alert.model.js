...
import { runInBackground } from 'mongoose-kue';
...
AlertSchema.pre('validate', function validate(next) {
  // ensure jurisdictions, receivers & methods

  // ensure statistics
  if (_.isEmpty(this.statistics) && !_.isEmpty(this.methods)) {
    const statistics = {};
    _.forEach(this.methods, function loopMethods(method) {
...
AlertSchema.methods.send = function send(done) {
  ...
  const jurisdictions = _.map(
    [].concat(this.jurisdictions),
    function cbFunction(jurisdiction) {
  ...
  const works = {};
  _.forEach(receivers, function cbReceiver(receiver) {
